      ...
      try {
        ...
        if (errorParam) {
          const errorDescription =
            queryParams.get("error_description") || "Unknown error";
          throw new Error(
            `Authorization server error: ${errorParam} - ${errorDescription}`
          );
        }
        ...
        if (!stateParam || !codeParam) {
          throw new Error(
            "Missing required OAuth callback parameters (state and/or code)"
          );
        }
        ...
        if (!isAuthenticated) {
          throw new Error(
            "Failed to complete authentication. Could not establish a valid session."
          );
        }
        ...
        try {
          ...
          console.warn(
            "Could not retrieve launch context, but authentication succeeded:",
            contextError
          );
